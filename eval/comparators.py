"""Answer comparison functions for CoT trajectory evaluation.

Math grading functions are copied from lm-evaluation-harness:
  lm_eval/tasks/score/math/math_grader.py  (Apache-2.0 / MIT / MIT)
  Original authors: NVIDIA, Microsoft (ToRA), OpenAI (prm800k), Dan Hendrycks

QA F1 functions are copied from:
...
# ===================================================================
# Math grading — copied from math_grader.py (lines 91-654)
# ===================================================================

def _check_antlr_version():
    """Check that the antlr package version matches what sympy LaTeX parser needs."""
    ...
    try:
        ...
        if installed_version != REQUIRED_VERSION:
            raise RuntimeError(
                f"Package {PACKAGE_NAME} version mismatch: {installed_version} "
                f"(required: {REQUIRED_VERSION})"
            )
    except PackageNotFoundError:
        raise RuntimeError(
            f"Package {PACKAGE_NAME} not found. "
            "Please install antlr4-python3-runtime==4.11.0."
        )
...
def normalize_answer_string(expr: str) -> str:
    ...
    for unit in [
        ...
        "hour", "week", "month", "year", "foot", "feet", "inch", "yard",
        "p.m.", "PM",
    ]:
...
# ===================================================================
# QA F1 — copied from longbench/metrics.py (lines 40-248)
# ===================================================================
...
def _normalize_qa_answer(s: str) -> str:
    ...
    def remove_punc(text):
        exclude = set(string_module.punctuation)
...
def exact_match(prediction: str, reference: str) -> bool:
    """Case-insensitive exact string match after stripping."""
    return prediction.strip().lower() == reference.strip().lower()


def normalized_text_match(prediction: str, reference: str) -> bool:
    """Normalized text match: lowercase, remove punctuation/articles/whitespace."""
    return _normalize_qa_answer(prediction) == _normalize_qa_answer(reference)
