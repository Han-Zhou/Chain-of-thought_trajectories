...
def generate_trajectories(model_name, dataloader, max_new_tokens, dataset_name=None, shot_mode="zero", thinking=False, confidence=None, debug=False):
    ...
    for i, entry in enumerate(dataloader):
        ...
        if debug:
            ...
            gen: GenerationResult = llm.generate_one(messages, max_new_tokens=max_new_tokens, output_scores=bool(confidence))
        ...
        if confidence and answer_token_start_position is not None:
...
def main(args):
    ...
    out_dir = f"trajectories/{args.model}_{'thinking' if args.thinking else 'regular'}_{args.dataset}_{args.shot_mode}_{sample_size}_{'conf' if args.confidence else 'vanilla'}"
